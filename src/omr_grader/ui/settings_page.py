...
class SettingsPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.path_card = QFrame(self)
        self.path_card.setObjectName("settingsPortablePathCard")
        path_layout = QVBoxLayout(self.path_card)
        self.path_heading = QLabel("1. 포터블 저장 위치", self.path_card)
        self.path_heading.setProperty("role", "card-title")
        path_layout.addWidget(self.path_heading)
        path_form = QFormLayout()
        self.data_path_edit = QLineEdit(self.path_card)
        ...
        profile_card_layout = QVBoxLayout(self.profile_card)
        self.profile_heading = QLabel("2. 기본 OMR 프로필", self.profile_card)
        self.profile_heading.setProperty("role", "card-title")
        profile_card_layout.addWidget(self.profile_heading)
        profile_form = QFormLayout()
        self.profile_combo = QComboBox(self.profile_card)
        self.profile_combo.setObjectName("settingsProfileCombo")
        self.profile_combo.setAccessibleName("기본 OMR 프로필")
        self.profile_import_button = QPushButton("OMR 프로필 불러오기", self.profile_card)
        ...
        profile_row = QWidget(self.profile_card)
        profile_layout = QHBoxLayout(profile_row)
        profile_layout.setContentsMargins(0, 0, 0, 0)
        profile_layout.addWidget(self.profile_combo, 1)
        profile_layout.addWidget(self.profile_import_button)
        self.profile_diagnostic_label = QLabel("검증된 프로필을 선택하세요.", self.profile_card)
        ...
        profile_form.addRow("기본 OMR 프로필", profile_row)
        profile_form.addRow("프로필 상태", self.profile_diagnostic_label)
        profile_card_layout.addLayout(profile_form)
        root.addWidget(self.profile_card)

        self.recognition_card = QFrame(self)
        self.recognition_card.setObjectName("settingsRecognitionCard")
        recognition_layout = QVBoxLayout(self.recognition_card)
        self.recognition_heading = QLabel("3. 인식 및 성능", self.recognition_card)
        self.recognition_heading.setProperty("role", "card-title")
        recognition_layout.addWidget(self.recognition_heading)
        recognition_form = QFormLayout()
        self.sensitivity_slider = QSlider(Qt.Orientation.Horizontal, self.recognition_card)
        ...
        self.sensitivity_label = QLabel("인식 수준 3 / 10", self.recognition_card)
        self.sensitivity_label.setObjectName("settingsSensitivityValue")
        sensitivity_row = QWidget(self.recognition_card)
        ...
        self.multiprocessing_checkbox = QCheckBox(
            "병렬 처리(멀티프로세싱) 사용", self.recognition_card
        )
        ...
        recognition_form.addRow("기본 인식 감도", sensitivity_row)
        recognition_form.addRow("성능 설정", self.multiprocessing_checkbox)
